...
本プロジェクトの HTTP リクエストライブラリ (`urllib3`) をメジャーバージョンアップしても、テストフレームワーク (`vcrpy`, `pytest-recording`) との互換性を保証するための段階的移行計画です。
...
- `types-urllib3`: v1.26.25.14 → urllib3に含まれているので削除
...
| types-urllib3 型定義 | 使用中 | ⚠️ 中 | urllib3 v2.xに含まれているので削除 |
...
| types-urllib3 | 1.26.25.14 | 1.26.25.14 | urllib3 v2.x に含まれているので削除 |
...
#### 2. types-urllib3 は urllib3 v2.x で不要

**理由**: urllib3 v2.x は py.typed を含み、インラインタイプ注釈を備えている

- v1.x: type stub ライブラリ (`types-urllib3`) で補完が必要
- v2.x: py.typed あり → インラインタイプ注釈を直接参照可能

**確認済み**: types-urllib3 削除後も mypy で型チェック成功、テストも全パス

**参考**: <https://pypi.org/project/types-urllib3/>

**教訓**: 型定義ライブラリは実装ライブラリと異なるメジャーバージョンサイクル。v2.x への移行時に必要性を再評価すること。
...
## 📝 追加調査結果：types-urllib3 の不要性（2025-12-14）

### 事実確認

urllib3 v2.x は `py.typed` ファイルを含み、インラインタイプ注釈を備えている：
...
- `types-urllib3==1.26.25.14` を `requirements.txt` から削除
- ユニットテスト実行: ✅ **9 passed in 0.06s**
- 型チェック実行 (mypy): ✅ **Success: no issues found in 8 source files**

### 結論

✅ **urllib3 v2.x では types-urllib3 は不要**

- urllib3 が py.typed を持つため、mypy などの型チェッカーはインラインのタイプ注釈を直接参照可能
- `types-urllib3` パッケージは urllib3 v1.x 用の type stub（v1.x にはインラインタイプ注釈がないため）

### 推奨事項

- pyproject.toml から `types-urllib3` の記載は不要（既に記載されていない）
- requirements.txt から `types-urllib3` を削除（完了）
- 他のプロジェクトも urllib3 v2.x 対応時に `types-urllib3` 削除を検討
